...
uint8_t usb0_rx_mem[USB_OUT_RINGBUFFER_SIZE] __attribute__((section(".system_ram")));
uint8_t usb1_rx_mem[USB_OUT_RINGBUFFER_SIZE] __attribute__((section(".system_ram")));
uint8_t uart0_rx_mem[UART_RX_RINGBUFFER_SIZE] __attribute__((section(".system_ram")));
uint8_t uart1_rx_mem[UART_RX_RINGBUFFER_SIZE] __attribute__((section(".system_ram")));

uint8_t src0_buffer[UART_TX_DMA_SIZE] __attribute__((section(".tcm_code")));
uint8_t src1_buffer[UART_TX_DMA_SIZE] __attribute__((section(".tcm_code")));
...
Ring_Buffer_Type usb0_rx_rb;
Ring_Buffer_Type usb1_rx_rb;
...
void uart0_init(void)
{
    ...
    //device_control(uart1, DEVICE_CTRL_ATTACH_TX_DMA, dma_ch3);
...
void uart1_dtr_pin_set(uint8_t status)
{
    gpio_write(uart1_dtr, status);
}
void uart1_rts_pin_set(uint8_t status)
{
...
void uart0_dtr_pin_set(uint8_t status)
{
    gpio_write(uart0_dtr, status);
}
void uart0_rts_pin_set(uint8_t status)
{
...
void uart1_ringbuffer_init(void)
{
    ...
    memset(usb1_rx_mem, 0, USB_OUT_RINGBUFFER_SIZE);
    memset(uart1_rx_mem, 0, UART_RX_RINGBUFFER_SIZE);

    /* init ring_buffer */
    Ring_Buffer_Init(&usb1_rx_rb, usb1_rx_mem, USB_OUT_RINGBUFFER_SIZE, ringbuffer_lock, ringbuffer_unlock);
    Ring_Buffer_Init(&uart1_rx_rb, uart1_rx_mem, UART_RX_RINGBUFFER_SIZE, ringbuffer_lock, ringbuffer_unlock);
}

void uart0_ringbuffer_init(void)
{
    /* init mem for ring_buffer */
    memset(usb0_rx_mem, 0, USB_OUT_RINGBUFFER_SIZE);
    memset(uart0_rx_mem, 0, UART_RX_RINGBUFFER_SIZE);

    /* init ring_buffer */
    Ring_Buffer_Init(&usb0_rx_rb, usb0_rx_mem, USB_OUT_RINGBUFFER_SIZE, ringbuffer_lock, ringbuffer_unlock);
    Ring_Buffer_Init(&uart0_rx_rb, uart0_rx_mem, UART_RX_RINGBUFFER_SIZE, ringbuffer_lock, ringbuffer_unlock);
}

static dma_control_data_t uart0_dma_ctrl_cfg =
{
    .bits.fix_cnt = 0,
    .bits.dst_min_mode = 0,
    .bits.dst_add_mode = 0,
...
static dma_control_data_t uart1_dma_ctrl_cfg =
{
    .bits.fix_cnt = 0,
    .bits.dst_min_mode = 0,
    .bits.dst_add_mode = 0,
...
static dma_lli_ctrl_t uart1_lli_list = 
{
    .src_addr = (uint32_t)src1_buffer,
    .dst_addr = DMA_ADDR_UART1_TDR,
    .nextlli = 0
};

static dma_lli_ctrl_t uart0_lli_list = 
{
    .src_addr = (uint32_t)src0_buffer,
...
{
    if(Ring_Buffer_Get_Length(&usb0_rx_rb))
    {
        if (!device_control(dma_ch3, DMA_CHANNEL_GET_STATUS, NULL))
        {
            uint32_t avalibleCnt = Ring_Buffer_Read(&usb0_rx_rb, src0_buffer, UART_TX_DMA_SIZE);
            
            if (avalibleCnt)
            {
                dma_channel_stop(dma_ch3);
                uart0_dma_ctrl_cfg.bits.TransferSize = avalibleCnt;
                memcpy(&uart0_lli_list.cfg, &uart0_dma_ctrl_cfg, sizeof(dma_control_data_t));
                device_control(dma_ch3,DMA_CHANNEL_UPDATE,(void*)((uint32_t)&uart0_lli_list));
                dma_channel_start(dma_ch3);
                led_toggle(0);  //TX indication
            }
        }
    }
    if(Ring_Buffer_Get_Length(&usb1_rx_rb))
    {
        if (!device_control(dma_ch2, DMA_CHANNEL_GET_STATUS, NULL))
        {
            uint32_t avalibleCnt = Ring_Buffer_Read(&usb1_rx_rb, src1_buffer, UART_TX_DMA_SIZE);
            
            if (avalibleCnt)
            {
                dma_channel_stop(dma_ch2);
                uart1_dma_ctrl_cfg.bits.TransferSize = avalibleCnt;
                memcpy(&uart1_lli_list.cfg, &uart1_dma_ctrl_cfg, sizeof(dma_control_data_t));
                device_control(dma_ch2,DMA_CHANNEL_UPDATE,(void*)((uint32_t)&uart1_lli_list));
